...
# Step 3 : Membaca Dataset HR_comma_sep.csv
HR_DS = read.csv('https://raw.githubusercontent.com/arikunco/machinelearning/master/dataset/HR_comma_sep.csv')
...
HR_DS$left <- as.factor(HR_DS$left)
train <- sample(1:nrow(HR_DS),as.integer(0.7*nrow(HR_DS)))
traindata <- HR_DS[train,]
testdata <- HR_DS[-train,]
